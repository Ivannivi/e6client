...
function getHostNames(settings: Settings): string[] {
  const hosts = ['e621.net', 'e926.net'];
  const activeAccount = getActiveAccount(settings);
  if (activeAccount?.hostUrl) {
    try {
      hosts.unshift(new URL(activeAccount.hostUrl).hostname);
    } catch {
      // Keep defaults if the stored host URL is invalid.
    }
  }
  return hosts;
}
...
export function useDeepLinks(settings: Settings) {
  ...
  useEffect(() => {
    const hosts = getHostNames(settings);
